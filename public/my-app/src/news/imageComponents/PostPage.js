...
const PostPage = () => {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/api/postfile/${id}`);
  ...
  const handleLike = async () => {
    try {
      const response = await axios.post(`http://localhost:5000/api/postfile/${id}/like`);
      setPost(response.data);
    } catch (error) {
  ...
  const handleComment = async () => {
    if (handleValidation()) {
      try {
        const response = await axios.post(`http://localhost:5000/api/postfile/${id}/comment`, {
          text: newComment,
        });
        setPost(response.data);
        setComments(response.data.comments);
